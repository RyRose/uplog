...
-- name: RawInsertSchedule :one
INSERT INTO schedule (date, workout, notes)
VALUES (?, ?, ?)
RETURNING *;

-- name: RawSelectSchedule :many
SELECT * FROM schedule;

-- name: RawSelectSchedulePage :many
SELECT * FROM schedule LIMIT ? OFFSET ?;

-- name: RawDeleteSchedule :exec
DELETE FROM schedule
WHERE date = ?;

-- name: RawUpdateScheduleDate :exec
UPDATE schedule
SET date = @out
WHERE date = @in;

-- name: RawUpdateScheduleWorkout :exec
UPDATE schedule
SET workout = ?
WHERE date = ?;

-- name: RawInsertScheduleList :one
INSERT INTO schedule_list (id, day, workout)
VALUES (?, ?, ?)
RETURNING *;

-- name: RawSelectScheduleList :many
SELECT * FROM schedule_list;

-- name: RawSelectScheduleListPage :many
SELECT * FROM schedule_list
ORDER BY id, day ASC
LIMIT ? OFFSET ?;

-- name: RawDeleteScheduleList :exec
DELETE FROM schedule_list
WHERE id = ?;

-- name: RawUpdateScheduleListId :exec
UPDATE schedule_list
SET id = @out
WHERE id = @in;

-- name: RawUpdateScheduleListDay :exec
UPDATE schedule_list
SET day = ?
WHERE id = ?;

-- name: RawUpdateScheduleListWorkout :exec
UPDATE schedule_list
SET workout = ?
WHERE id = ?;
